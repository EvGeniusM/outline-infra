...
Plane (Community Edition) поднимается в том же стеке и доступен на
**http://plane.localhost**. Переиспользует общие Postgres (БД `plane`),
Redis (db 1) и MinIO (бакет `plane-uploads`); единственный новый инфра-сервис —
RabbitMQ (брокер Celery).

### Первый запуск (обязательные ручные шаги)

1. `make up` (или `docker compose up -d`), дождаться, пока `plane-app` поднимется
   и прогонит миграции: `docker compose logs -f plane-app`.
2. Открыть **http://plane.localhost** → пройти мастер: создать **instance-admin**.
3. Зайти в **http://plane.localhost/god-mode** → раздел Authentication →
   убедиться, что включён вход по **email + паролю** (включается автоматически,
   т.к. SMTP не настроен), и выставить нужный режим регистрации
   (открытая саморегистрация vs только по приглашению).
4. Завести пользователей с **теми же email**, что в Outline/Dex.

### Единые пользователи

Единого SSO между Outline и Plane нет: generic-OIDC (вход через Dex) в Plane —
платная фича Pro/Business. Бесплатно «одни и те же юзеры» обеспечиваются
заведением аккаунтов с одинаковыми email. Outline по-прежнему логинится через Dex.

### Русский язык интерфейса

Язык в Plane — пер-юзерный (fallback зашит «en»). Каждый пользователь один раз
выбирает «Русский» в **Profile → Preferences → Language**; выбор сохраняется.

### SMTP

По умолчанию не настраивается: нет email-приглашений (ссылки на инвайты
раздаются вручную), сброса пароля и уведомлений. Настраивается позже в `/god-mode`.

### Бэкап

`make backup` / `make restore ARCHIVE=...` уже включают БД `plane` и бакет
`plane-uploads` — отдельных действий не требуется.
...
Четыре A-записи на публичный IP VM: `app.<domain>`, `auth.<domain>`, `files.<domain>`, `plane.<domain>`. `files.*` обязателен — браузер ходит туда напрямую за аватарками/файлами (см. «Конфигурация хранилища»).
...
`gen-secrets.sh` генерирует `SECRET_KEY`/`UTILS_SECRET` и `PLANE_SECRET_KEY`/`LIVE_SERVER_SECRET_KEY` — последние **не ротировать** после первого запуска Plane. Остальные плейсхолдеры из `.env.example` (`POSTGRES_PASSWORD`, `MINIO_ROOT_PASSWORD`/`AWS_SECRET_ACCESS_KEY`, `OIDC_CLIENT_SECRET`, `RABBITMQ_PASSWORD`) нужно руками заменить на новые случайные значения (`openssl rand -hex 24`) — не те, что использовались локально.
...
- Сайт-блоки — без `http://`, с реальными доменами (`app.<domain>`, `auth.<domain>`, `files.<domain>`, `plane.<domain>`); Caddy сам получит сертификаты Let's Encrypt по HTTP-01. Plane-блок: заменить `http://plane.localhost` на `plane.<domain>`, обновить `PLANE_URL` в `.env`.
